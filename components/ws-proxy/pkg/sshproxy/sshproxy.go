...
func NewProxy(ip proxy.WorkspaceInfoProvider, podConfig *proxy.WorkspacePodConfig) *Proxy {
	var res Proxy
	res.InfoProvider = ip
...
func (p *Proxy) handleChannel(conn *ssh.ServerConn, newChan ssh.NewChannel) {
	...
	addr, err := buildWorkspacePodURL(p.PodConfig.ServiceTemplate, workspaceID, fmt.Sprint(p.PodConfig.SSHPort))
	if err != nil {
